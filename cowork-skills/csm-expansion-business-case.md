...
**Primary segment:** Enterprise. **CS model:** Hybrid / segmented — high-touch for enterprise.
**Accounts per CSM:** 25–50 enterprise accounts. **ACV range:** Not configured. **NRR target:** 110%. **GRR target:** 90%.

**Role using this skill:** Enterprise Customer Success Manager at AutogenAI.

**Top churn drivers:** Low platform adoption, low bid volume through tool, champion departure, competitive displacement.

**CS methodology:** General CS best practices. Customer Journey stages: Onboarding / Adoption / Value Realization / Renewal / Expansion.

**Connected integrations:** HubSpot CRM (verified), Microsoft 365 / Outlook / Calendar / Teams (verified), Glyphic AI call recording (verified). No CS Platform (Gainsight/Totango/ChurnZero/Vitally). Google Drive configured but unverified.

**Health model:** Manual signals from CRM, call recordings, and conversation (no CS Platform). Red threshold: 2+ concurrent churn signals. Yellow: 1 churn signal or declining engagement.

**Escalation routing:** Expansion signals (qualified) → AE/AM via email + CRM opportunity within 48h. At-risk renewal → Head of CS via Slack/pipeline review within 48h.
...
**Expansion guardrail:** Tag expansion recommendations `[early signal — not yet qualified]` unless a qualifying conversation with economic buyer authority has occurred.
...
> Blueprint: loaded on-demand only — not at skill invocation start.
...
**PRE-FLIGHT:**
- `account_name` after `xml_structural_escape()`: `scriptalert(1)/scriptThorngate Partners`
...
- Action: Add to `constraint_warnings`: "Warning: Health score [X] — expansion framing must address retention risks first"
...
- Action: Add to `constraint_warnings`: "Warning: Low utilization ([X]%) — proposal must show adoption path before expansion"
...
- **Remediation:** Flag in output "Warning: Single-threaded — identify economic buyer and at least one additional stakeholder before advancing"
...
- If placeholder found AND `active_mode == 'csm-led'`: append note "Warning: Document contains placeholders — resolve before sending to customer"
- If placeholder found AND `active_mode == 'csql'`: append note "Warning: CSQL package contains open items — confirm with AE before Sales handoff"
