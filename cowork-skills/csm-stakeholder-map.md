...
**CS Role:** Enterprise Customer Success Manager. CS model is hybrid/segmented — high-touch for enterprise (the primary segment covered). Default book: 25–50 enterprise accounts.

**Connected integrations:** HubSpot CRM (verified), Glyphic AI call recording (verified), Microsoft 365 (verified). No CS Platform (Gainsight/Totango/etc.) — health signals come from CRM + call recordings + conversation.

**Top churn drivers:** Low platform adoption, low bid volume through tool, champion departure, competitive displacement.

**Health model:** Sponsor engagement is a health component (20% weight). Red threshold: 2+ concurrent churn signals. Yellow: 1 churn signal or declining engagement.

**Escalation matrix defaults:** Red health → manager (24h); at-risk renewal → Head of CS (48h); executive escalation → manager + AE (same day); expansion signal → AE/AM (48h).
...
**Data freshness rule:** Every output that draws on CRM data states the data-as-of timestamp. If data is more than 7 days old, surface it.
...
Apply the Company Context embedded above. Note from config:
- CS motion is high-touch for enterprise accounts — this requires deeper stakeholder coverage (minimum: executive sponsor + champion + end-user representative)
- Escalation matrix is configured in Company Context above — use it for sponsor risk routing
- Health model includes sponsor engagement at 20% weight; apply configured thresholds

**G-code guardrails** (inline, since config is embedded):
- G1: Do not classify accounts as likely to churn or assign churn probability — present component signals only
- G4: Do not recommend escalation without a named escalation path (see escalation matrix in Company Context)
...
2. **CONSTRAINTS**: What limits the solution space?
   - CS motion (high-touch for enterprise) determines minimum role coverage and dormancy thresholds
   ...
   - Escalation matrix must be configured before routing sponsor risk actions — see Company Context
...
**For complex scenarios**, apply the reasoning blueprint in the Reference Material section below.
...
- CRM (HubSpot — verified): contact list with titles, departments, roles, last activity date
- Call recording (Glyphic AI — verified): attendees across last 3-5 calls — who's been on calls, who's dropped off
- CS Platform: not connected — omit; note absence in reviewer note
...
For high-touch motion (enterprise): aim for at minimum executive sponsor + champion + end-user
...
**Engagement health thresholds (high-touch enterprise motion):**
- Declining = >30 days no contact; Dormant = >60 days no contact
- Adjust thresholds for executive vs. operational contacts (executives quarterly = healthy; champion 30-day silence = declining)
...
> **⚠️ Reviewer note**
> - **Sources:** [CRM — HubSpot ✓ live — [N] contacts retrieved | CRM configured but unverified | call recording — Glyphic AI: attendees from last [N] calls | user provided | not connected — conversation context only]
...
Stakeholder map — format driven by `--map` (default) or `--sponsor-risk` flag.
