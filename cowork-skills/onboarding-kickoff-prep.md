...
**Role:** Enterprise CSM. Team structure: Enterprise accounts are served by CSM + Onboarding PM + Implementation Consultant + Bid Consultant. Mid-Market and SME are IC-led with handoff back to CSM.

**Segments:** Enterprise (primary); Mid-Market and SME (lighter model).
...
**Kickoff format:** Spans Initiation + Discovery (Month 1) — stakeholder ID, kick-off call, bid discovery session, value alignment session, project rollout planning. Customer-facing plan format: PowerPoint deck.

**Required attendees (Enterprise, client-side):** Client PM/Project Leads, Internal Champions, Bid/Proposal Reps, Key End Users, Executive Sponsors.

**Success framework:** Triple Metric — Corporate (Win rate, Revenue, ROS/ROA), Business unit (Technical score, Operating margins, Revenue per transaction), Project (Time saved, User adoption, Headcount efficiency). Success criteria are defined jointly during the Value Alignment Session in Month 1 and reviewed quarterly.

**Key milestones (Enterprise):** M1 = Kickoff + Discovery complete (Month 1); M2 = Configuration complete (Month 1); M3 = Adoption underway (Months 2–4); M4 = First Business Review (Month 3–4); M5 = Handoff ready (Month 5).

**Tools:** CRM = HubSpot; CS/PM = Planhat; Docs = SharePoint; Comms = Slack + Outlook.
...
Fields drawn from the embedded Company Context above:
- **Kickoff format:** Spans Initiation + Discovery (Month 1) — stakeholder ID, kick-off call, bid discovery session, value alignment session, project rollout planning
- **Required kickoff attendees (Enterprise):** Client PM/Project Leads, Internal Champions, Bid/Proposal Reps, Key End Users, Executive Sponsors
- **Onboarding model:** White-glove (Enterprise), Implementation-plus-handoff (Mid-Market/SME)
- **Milestone framework:** Month-based (see Company Context above); M1 day target = [PLACEHOLDER — not yet configured]
- **Success criteria review cadence:** Quarterly
- **CS methodology:** None formal
- **Customer-facing plan format:** PowerPoint deck

If any of these fields contain `[PLACEHOLDER]` markers, surface every default visibly before generating output. Silent genericity is the primary risk.

If kickoff-related fields are missing:
> "Kickoff format and required attendees aren't fully configured. The output will note which fields are defaulted — review before sharing."

Proceed with a general structure if the user confirms they want to continue despite missing config.

**G-code dependency:** All G-code guardrails referenced in this skill (G1–G9) are operational only when config is complete. If critical config is missing, do not proceed with partial config without flagging every gap.
...
- Customer-facing plan format note (e.g., "We'll share your onboarding plan as a PowerPoint deck after this call")

### Model adaptations

**white-glove (Enterprise):** Expand "Partnership vision" section; include an executive
sponsor introduction segment if the exec is attending. Add a dedicated
slide/section for "Our team's commitment to you" framing. Name the full AutogenAI team (CSM, Onboarding PM, IC, Bid Consultant) and clarify each role. Include the Value Alignment Session and Triple Metric introduction in the First Priorities section.
...
**implementation-plus-handoff (Mid-Market/SME):** Add an "Implementation phase overview"
section between milestones and first priorities. Name the implementation
consultant and distinguish their role from the CSM's role.
...
- [ ] Client PM/Project Lead confirmed attending — name: [name] / not yet confirmed
- [ ] Internal Champion confirmed — name: [name] / not yet confirmed
- [ ] Bid/Proposal Rep confirmed — name: [name] / not yet confirmed
- [ ] Key End User(s) confirmed — name(s): [name] / not yet confirmed
- [ ] Executive Sponsor confirmed (required for white-glove Enterprise)
- [ ] AE attending or sent intro message to customer team
- [ ] IC named and introduced (if implementation-plus-handoff)
...
- [ ] Onboarding plan PowerPoint deck drafted (or template ready to complete during/after call)
- [ ] Value Realisation Deck template accessible (stored at `/Users/adelinamanea/Downloads/NEW TEMPLATE - Value Realisation Deck.pptx`)
- [ ] Triple Metric framing prepared for Value Alignment Session discussion
...
- [ ] Known stakeholders mapped: executive sponsor / champion / bid reps / technical lead / billing contact

### 4. First milestone setup

- [ ] M1 target date confirmed: [contract start + M1 day target from config — currently PLACEHOLDER, confirm with account]
- [ ] M1 completion criteria clear: Stakeholders confirmed, end users finalised, bid discovery done, Value Alignment Session complete, Triple Metric agreed
- [ ] First follow-up meeting scheduled (or to be scheduled on the kickoff call)
- [ ] Planhat task/project created for this account

### 5. Post-kickoff actions (prepare before the call)

- [ ] Draft follow-up email ready (to send same day with next steps)
- [ ] Onboarding plan PowerPoint link ready to share
- [ ] Value Realisation Deck or success criteria template ready to share
- [ ] Introductions to IC and Bid Consultant drafted if needed
...
**AutogenAI × [Account Name] — Onboarding Kickoff**
*[Date] · [Time and timezone] · [Format: video call / in-person / async]*

**Attendees from AutogenAI:** [CSM name], [AE name if attending], [Onboarding PM], [IC name if applicable]
**We'll need from your team:** Client PM/Project Leads, Internal Champions, Bid/Proposal Reps, Key End Users, Executive Sponsors
...
**First milestone target:** [M1 target date] — Kickoff + Discovery complete (stakeholders confirmed, bid discovery done, Value Alignment Session complete, Triple Metric agreed)

**Your onboarding plan** will be shared as a PowerPoint deck following this call.
...
> - **Before sending agenda:** Remove this reviewer note block. Confirm AutogenAI
>   appears correctly in the header. Confirm the M1 target date is accurate
...
between onboarding models. If the model is not determinable, default to the
white-glove structure (Enterprise) or implementation-plus-handoff (Mid-Market/SME) and flag it — do not silently produce a generic agenda.
...
*For use when Tier 3 reasoning is activated for onboarding kickoff preparation.*
...
Primary Risk: Producing a generic agenda that ignores the onboarding model adaptation — white-glove and implementation-plus-handoff require materially different agendas.
...
- Model label without adaptation: Agenda says "white-glove" in metadata but structure is identical to implementation-plus-handoff.
...
- Missing partner escalation path: Customer has no documented way to reach AutogenAI directly if the partner is unresponsive.
...
- If config is heavily incomplete (3+ placeholder fields), recommend completing config first rather than generating a low-quality artifact.
...
- Pull CRM data from HubSpot before asking the CSM for manual input — avoid asking for information that's already available.
