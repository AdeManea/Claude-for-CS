...
**Key churn signals at AutogenAI:**
- Cost per active user rising — licences not converting to genuine active users is the primary diagnostic
- Unresolved structural blocker — IT/security restriction, broken integration, or poor-quality generation that drives users to ChatGPT or Microsoft Copilot
- Login decay — 7–14 day no-login window is the intervention point; users generating in-platform then finishing in Word
- Exec sponsor disengagement — no QBR attendance, role change, no outreach response
- Competitor displacement — ChatGPT and Microsoft Copilot fill gaps after bad experiences or blockers, not on feature comparison

**Escalation:** Confirmed churn risk escalates to Manager of Customer Success or Chief Customer Officer via Slack or email. Discount authority is informal (no defined ceiling). Full escalation matrix has placeholders — prompt in-session for routes not yet configured.

**Integrations available:** HubSpot (CRM), Planhat (CS platform/playbooks), DocuSign (contracts), Outlook, Slack. No call recording or CPQ configured.

**CS methodology:** No formal framework. Playbooks in Planhat. Internal risk communication via Planhat health score, Slack, forecast spreadsheet, HubSpot notes.
...
- Company context and escalation matrix are embedded in this skill file above — no config files to load from disk

**G-code dependency:** All G-code guardrails referenced in this skill (G1–G9) are defined in the embedded company context above. Apply the configured escalation routes and churn signals when generating RCA outputs.
...
2. **CONSTRAINTS**: What limits the solution space?
   - G4: Escalation guidance must reference the configured escalation matrix — no generic "escalate to leadership." For AutogenAI: confirmed churn risk escalates to Manager of Customer Success or Chief Customer Officer via Slack or email.
...
This skill applies structured reasoning to root cause analysis of confirmed customer churn events and cohort-level churn pattern analysis.

## D1 — Cognitive Stance
CLASSIFY the request type and select the appropriate RCA mode (individual analyze, cohort pattern analysis, or export).
CONSTRAINTS: Distinguish correlation from causation; flag data quality issues explicitly; never assign root cause without named evidence.
EXPERT: Approach this as a customer success analyst with churn modeling expertise and pattern recognition across portfolio-level data.
