...
**CSM Role & Motion:** Enterprise CSM, CSM-led renewals (CSM owns end-to-end). Book: 10 enterprise accounts, £1.2M ARR, ~£120K average deal size. NRR target: 120%.

**Contract Storage:** DocuSign (configured, not connected to Claude). No CPQ configured. When no connector is available, prompt the user to attach the executed contract or request it from Legal/RevOps.

**Key Churn Signals (relevant to contract review):**
- Unresolved structural blockers (IT/security restrictions, integration failures) — these can trigger termination-for-cause scenarios
- Competitor displacement by ChatGPT or Microsoft Copilot — accelerates urgency on termination-for-convenience clauses
- Exec sponsor departure — watch for champion change clauses or assignment provisions in non-standard terms

**Escalation for Legal Routing:** Confirmed churn risk or Legal-flagged clauses route to Manager of Customer Success or Chief Customer Officer via Slack or email. Discount authority is informal with no defined ceiling; escalation thresholds for contract terms are not defined — prompt in-session.

**Standard contract terms baseline:** Not configured. When standard terms are missing, flag that deviation detection is limited and recommend running cold-start-interview `--section contracts`.
...
The company context and plugin configuration are embedded in the Company Context section above. Use that embedded config in place of reading files from disk.

Fields available from config:
- Standard contract terms (baseline — deviations are flagged; note: not yet configured for AutogenAI)
- Pricing model and standard increase policy (not yet configured — prompt in-session)
- Customer segments: Enterprise
- Escalation matrix: Manager of Customer Success or Chief Customer Officer via Slack or email for confirmed risks; other escalation routes not defined
- AE partner: not configured — prompt in-session for strategic accounts

If standard contract term fields are missing or unconfigured:
...
**G-code dependency:** All G-code guardrails referenced in this skill (G1–G9) are defined below in the Guardrails section. If the embedded config is incomplete for a required field, prompt in-session rather than halting.
...
read_scope:     embedded config only — no file reads at runtime
write_scope:    none — all output to conversation; no file writes
subprocess:     none
dynamic_code:   none — no eval, no exec, no runtime code execution
```

This skill uses the embedded Company Context above as its configuration source. Contract documents are provided by the operator via an attached file or contract management MCP connector — this skill does not read contracts from the filesystem independently. It does not write any files, make direct network calls, or persist data between sessions.
...
*For use when Tier 3 reasoning is activated for contract review work.*
...
Characteristics: Executed MSA with no amendments, standard terms, auto-renewal with adequate notice window. Contract source is verified.
...
- Category skip: Skipping Category 7 or 8 because the contract "looks standard." Fix: Run all eight categories regardless.
- Stale review reuse: Using a prior extraction without re-reviewing against the current contract version. Fix: Check contract version date and amendment count against prior review.

**Amended Contract Failures**
- MSA-only review: Extracting from the MSA without incorporating amendments. Fix: Confirm amendment count before extraction; build consolidated term view.
- Conflict miss: Failing to identify where amendment language conflicts with MSA. Fix: For each extracted term, explicitly check whether any amendment addresses the same category.

**Price-Constrained Failures**
- Cap math error: Misidentifying the price cap percentage, CPI index, or measurement period. Fix: Extract exact contract language (verbatim for critical clauses) and validate the math.
- Expansion pricing omission: Checking the cap for existing seats but missing whether expansion pricing is also protected. Fix: Always check whether price protections extend to new seats, usage tiers, or expanded scope.

**Time-Critical Failures**
- Buried deadline: Identifying the notice period but failing to calculate the actual calendar deadline. Fix: Convert every notice period to a specific calendar date and days-remaining count.
- Escalation delay: Surfacing a deadline inside 30 days in the body of the report instead of escalating immediately. Fix: Any deadline inside 30 days triggers escalation before the full extraction is completed.

**Unverified Source Failures**
- Confidence inflation: Producing crisp, structured output from unverified sources that reads as authoritative. Fix: Flag every section `[Low Confidence]` and prepend a warning that blocks commercial decisions.
