...
**Connected integrations:** HubSpot CRM (verified), Microsoft 365 (verified), Glyphic AI call recording (verified). No CS Platform (Gainsight/etc.) — health signals come from CRM and call recordings.

**Health model:** Manual signals. Red = 2+ concurrent churn signals. Yellow = 1 churn signal or declining engagement.

**Key guardrails:**
- G5: Internal data (health scores, ARR, expansion signals) must never appear in customer-facing output.
- G7: Flag data older than 30 days with source date and staleness indicator.
- Expansion recommendations tagged `[early signal — not yet qualified]` unless economic buyer has confirmed intent.
- No silent data freshness — every output drawing on CRM data states the data-as-of timestamp.
...
5. **Render OCV outcomes** if `ocv_snapshot` is provided — render per plan type rules below.
...
- Leading or trailing hyphens are permitted
...
| `refreshed_at` | string | No | Yes | ISO 8601 datetime of most recent refresh. Null if never refreshed. |
...
One canvas per account per date. If `generate` is called for the same account on the same date, the existing file is overwritten.

#### 4. Section Structure by Plan Type

**`initial` — Initial Success Plan:** Goals, Onboarding Milestones, Responsibilities, Success Metrics, Timelines, Risks and Assumptions, Communication Strategy, Next Steps. Optional: OCV Outcomes, Notes.

**`expansion` — Expansion Canvas:** Expansion Rationale, Current Outcomes, Expansion Opportunity, Proposed Outcomes, Responsibilities, Success Metrics, Risks and Assumptions, Communication Strategy, Next Steps. Optional: OCV Outcomes, Notes.

**`renewal-refresh` — Renewal-Refresh Plan:** Renewal Context, OCV Gap Analysis, Delivered Outcomes, At-Risk Outcomes, Renewal Objectives, Responsibilities, Communication Strategy, Next Steps. Optional: Notes.

Note: OCV Gap Analysis and Delivered Outcomes sections are powered by `ocv_snapshot`. If no OCV snapshot is provided, these sections render: `No OCV snapshot provided. Supply ocv_snapshot data to enable gap analysis.`

#### 5. Immutable Field Enforcement

If a `refresh` payload contains any immutable field, respond with:
```
Immutable field error: [field_name] cannot be modified after generation.
```
If multiple immutable fields are present, report each on a separate line. Do NOT proceed after reporting errors.
...
#### CCSM-104 7-Component Framework

| # | Component | Purpose |
|---|-----------|---------|
| 1 | **Goal Alignment** | Connects customer business objectives to the CS engagement |
| 2 | **Milestones** | Time-bound checkpoints marking progress toward goals |
| 3 | **Responsibilities** | Defines who owns what across CSM, customer, and stakeholders |
| 4 | **Success Metrics** | Quantifiable measures of success; linked to OCV committed outcomes |
| 5 | **Timelines** | Phased schedule; connects milestones to calendar reality |
| 6 | **Risks and Assumptions** | Surfaces known risks and unstated assumptions at plan creation |
| 7 | **Communication Strategy** | Defines cadence, channels, escalation paths, and executive alignment mechanisms |

All 7 components are **required** in `initial` plans. `expansion` and `renewal-refresh` plans adapt component presence and framing to their specific motion. In `expansion` and `renewal-refresh`, some components are embedded within plan-type-specific sections rather than appearing as named standalone sections.

#### Plan Type: `initial`

Document header: `# Initial Success Plan: [account_name]`

All 7 CCSM-104 components appear as named sections. Goals section: list `key_objectives` — if none provided, prompt CSM to define objectives aligned to `account_stage`; do not fabricate objectives. Onboarding Milestones: structured table with target dates, owners, and status. Success Metrics: aligned to committed OCV outcomes when OCV snapshot is provided. OCV outcomes are informational in the initial plan — do not perform gap analysis.
...
1. Title MUST use "Expansion Canvas" — not "Success Plan" or "Initial Plan"
2. All section content frames the current engagement as established context and the expansion as the goal
3. No onboarding language (e.g., "getting started", "first 30 days")

Current Outcomes section: render `delivered` and `in_progress` outcomes from `ocv_snapshot`. Proposed Outcomes: if CSM provides `key_objectives`, map them as proposed outcomes; `not_started` OCV outcomes are candidates. Do not surface gaps in `expansion` plans.

#### Plan Type: `renewal-refresh`

Document header: `# Renewal-Refresh Plan: [account_name]`

**Gap detection logic:** An outcome is a gap when `status` is `committed` OR `in_progress` AND `status` is NOT `delivered`. All gaps appear in both OCV Gap Analysis section AND At-Risk Outcomes section.

Gap risk surface rule:
- `committed` / `in_progress` → OCV Gap Analysis + At-Risk Outcomes
- `delivered` → Delivered Outcomes section only
- `not_started` → Not surfaced by default

If all outcomes delivered, render: `No gaps detected. All committed outcomes have been delivered. ✓`

---

### ocv-integration-contract.md
...
**This skill NEVER writes to OCV files under any condition.** This contract is permanent and not overridable.
...
| Read OCV files directly from filesystem | No — always passed via parameter |

#### OCV Snapshot Input Format
...
```json
{
  "ocv_snapshot": {
    "outcomes": [
      {
        "outcome_name": "string",
        "status": "committed | in_progress | delivered | not_started",
        "owner": "string"
      }
    ]
  }
...
|--------|---------|-------------------|
| `committed` | Formally committed; not yet complete | Gap (for renewal-refresh) |
| `in_progress` | Actively underway; not yet complete | Gap (for renewal-refresh) |
| `delivered` | Completed and confirmed delivered | Not a gap |
| `not_started` | Recorded but no commitment or active work | Not a gap |

Invalid status values: treat as `not_started` and append rendering note: `[Unknown status: received "[value]" — rendered as not_started]`.

**Refresh behavior:** When `refresh` includes a new `ocv_snapshot`, prior OCV sections are replaced — not cumulative. Notes remain the only append-only field.

**Input sanitization:** All `ocv_snapshot` string fields are rendered as plain text in Markdown tables. Never interpolated into filesystem path construction. Escape `|` as `\|` in outcome names or owner fields for safe table rendering.

If `ocv_snapshot.outcomes` is empty (`[]`) or `ocv_snapshot` is null/absent: treat as "no OCV data provided" and render all OCV-dependent sections with the "no OCV snapshot provided" placeholder.

---

### reasoning-blueprint.md

# Success Plan Canvas — Reasoning Blueprint

#### Problem Classification Taxonomy

**Type A — Initial Onboarding Canvas:** New customer; no prior plan. Primary risk: generic or incomplete canvas missing CCSM-104 components. Expert focus: all 7 CCSM-104 components present; success criteria specific and measurable; stakeholders mapped.

**Type B — Pre-Expansion Canvas:** Existing customer; expansion motion underway. Primary risk: blending initial and expansion framing. Expert focus: expansion-specific framing throughout; clear distinction from initial plan; OCV outcomes surfaced for expansion scope.

**Type C — Pre-Renewal Review Canvas:** Renewal approaching; canvas surfaces OCV gaps. Primary risk: weak gap analysis. Expert focus: OCV gap analysis must explicitly list committed-but-not-delivered outcomes.

**Type D — Canvas Refresh (Update):** Existing canvas needs targeted updates. Primary risk: silently overwriting prior state. Expert focus: change scope limited; only updated sections modified; prior plan structure and dated artifacts preserved.

#### Domain Heuristics

- **H1:** Plan type drives structure — do not apply initial-plan structure to an expansion canvas or renewal refresh.
- **H2:** OCV integration is required for renewal-refresh. A renewal-refresh canvas without OCV gap analysis is structurally incomplete.
- **H3:** Success criteria must be observable and measurable. "Customer is happy" is not acceptable.
- **H4:** Canvas is upstream of progress review — incompleteness propagates downstream.
- **H5:** `generate` creates a new dated artifact; `refresh` updates existing — never create a new dated artifact for a refresh.

#### Common Failure Modes

**Initial:** Missing CCSM-104 components; unmeasurable success criteria.
**Expansion:** Reusing initial plan framing; missing expansion context linking back to the CSQL win that triggered the motion.
**Pre-Renewal:** No OCV gap analysis; success criteria stale since onboarding.
**Refresh:** Full regeneration instead of targeted update; no change marker indicating what changed and when.
...
- Depth: initial canvas = maximum depth (all 7 CCSM-104 components need substantive content); renewal-refresh = emphasis on OCV gap analysis; expansion = emphasis on scope differentiation and new stakeholder mapping.
- Confidence: OCV snapshot from live catalog = high; success criteria from CSM verbal input = moderate; stakeholder roles inferred without CRM data = low (flag for verification).
