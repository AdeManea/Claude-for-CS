---
name: cs-ops-data-quality-check
description: >
...
**CS Team:** 9 CSMs across three segments — SME (<£50K ARR, 1:50 ratio), Mid-Market (£50K–£120K, 1:25), Enterprise (>£120K, 1:10). 1 dedicated CS Ops. Segment assignment is manual.

**Key metrics:** NRR target 120%, CSAT 4.5/5, cost per active user <£12K. Reporting is monthly and quarterly. Health model owned by CCO + CS Ops, reviewed ad hoc.

**Required fields — every account must have:**
Last contract renewal date, services commencement date, ARR, CSM owner, IC (Implementation Consultant), account segment (SME / Mid-Market / Enterprise), health score, autorenewal clause, number of licences, platform configuration information, paid add-ons, term length.

**Data quality standards:** Health data staleness threshold is 30 days. Health score audit is monthly (CS Ops). Broader data quality audit is ad hoc (CS Ops).

**Tooling:** CRM is HubSpot (configured, not tested). CS Platform is Planhat (configured, not connected to Claude). BI/Reporting is PowerBI (not connected). Call recording is Airspeed/Glyphic (configured). Support is Zendesk (not connected). Planhat is the source of truth for health data.
...
Apply configuration from the Company Context section above.

Critical configuration to apply:
- Required CRM fields and their completeness expectations (see Company Context — Required fields list)
- Health data staleness threshold: 30 days
- Segment assignment method: manual — segment field is always required
- CSM assignment model: direct ownership — CSM owner field is always expected

**G-code dependency:** All G-code guardrails referenced in this skill (G1–G9) apply as follows:
- G2: Staleness threshold is 30 days (configured above) — do not invent alternatives
- G4: Consistency violations may be legitimate manual overrides — flag, do not auto-correct
- G5: Remediation backlog requires named ownership and due dates
- G7: Every stale record flagged must include source date; every data gap must be quantified by ARR impact
...
- CRM (HubSpot): all account records with all field values (not just populated fields)
- CS Platform (Planhat): health scores, component scores, lifecycle stages, last-updated timestamps
- Required field list and staleness thresholds: see Company Context above
...
*Required fields audited: [N] (per cs-ops configuration)*
...
*Threshold: health scores updated within 30 days*

| Field | Staleness threshold | Stale accounts | Stale % | ARR in stale accounts |
|-------|-------------------|---------------|---------|----------------------|
| Health score | 30 days | [N] | [%] | $[amount] |
| Health score components | 30 days | [N] | [%] | $[amount] |
...
SME segment, which may indicate the tech-touch motion relies on manual score
updates that are not being performed. Enterprise accounts are all current.
Recommend an automated scoring trigger for SME accounts or a monthly forced
...
|-------|------|-----------|------------------|
| Segment ↔ ARR | Account ARR should match configured segment ARR range (SME <£50K, Mid-Market £50K–£120K, Enterprise >£120K) | [N] | $[amount] |
...
| CSM owner ↔ segment | CSM motion mismatches segment (e.g., SME CSM on Enterprise account) | [N] | $[amount] |
...
> |---------|-------------|-------------|---------|
> | [Account] | Segment: Enterprise | ARR: £35K | ARR below Enterprise floor of £120K |
>
...
| Integration fix (1+ week) | [N] | [e.g., Health score sync not firing for SME] |
...
> - **Required fields audited:** [Configured in Company Context above | User-provided this session]
> - **Staleness threshold applied:** 30 days (configured)
...
**Staleness thresholds are configured — do not invent them.** The configured
threshold is 30 days for health data. If no threshold exists for a given field,
flag the gap and recommend one; do not apply an arbitrary default without
surfacing it. The staleness section must state which threshold was applied.
...
*For use when deeper reasoning is required on complex or ambiguous audit requests.*
...
- Count-based false confidence: Reporting "95% complete" when the missing 5% holds 30% of ARR. Fix: Always compute ARR-weighted completeness alongside record-count completeness.
- Threshold invention: Applying staleness thresholds that aren't configured, producing arbitrary flags. Fix: State the configured threshold source explicitly. If none exists, flag the gap — don't default silently.

**Post-Transition Validation Failures**
- Attribution confusion: Blaming the transition for pre-existing data gaps. Fix: Compare last-updated timestamps against transition date to separate old from new issues.
- Mass assignment without investigation: Auto-assigning orphaned records to new CSMs without checking if accounts are active. Fix: Require disposition (assign/archive/investigate) per orphan, not bulk assignment.

**Analytics Investigation Failures**
- Symptom patching: Correcting the one bad record that caused a dashboard anomaly without checking for systemic cause. Fix: When a single record is wrong, query the same field across all records for the same failure pattern.
- Silent scope narrowing: Running only the mode that matches the symptom (e.g., --completeness) when the root cause is a consistency issue. Fix: Start with --full for investigations; narrow after root cause is identified.

**Recurring Audit Failures**
- Groundhog Day reports: Producing the same findings with no progress tracking. Fix: Include a "vs. prior audit" comparison showing remediation progress on previously flagged items.
...
**Severity Decisions**
- ARR concentration determines severity, not account count. A single enterprise account missing renewal date is higher priority than 20 SME accounts missing segment.
